...
function main() {
    ...
    faulDisplays.home.dom = document.querySelector('#home-faul');
    faulDisplays.guest.dom = document.querySelector('#guest-faul');
    add2array(clock.dom.querySelectorAll('.digit'), clock.digits);
    add2array(faulDisplays.home.dom.querySelectorAll('.digit'), faulDisplays.home.digits);
    add2array(faulDisplays.guest.dom.querySelectorAll('.digit'), faulDisplays.guest.digits);


    porDisplays.home.dom = document.querySelector('#home-por');
    porDisplays.guest.dom = document.querySelector('#guest-por');
    add2array(clock.dom.querySelectorAll('.digit'), clock.digits);
    add2array(porDisplays.home.dom.querySelectorAll('.digit'), porDisplays.home.digits);
    add2array(porDisplays.guest.dom.querySelectorAll('.digit'), porDisplays.guest.digits);    
    /**
     * 
    ...
    porDisplays.name.home = document.querySelector('#home-por-name');
    porDisplays.name.guest = document.querySelector('#guest-por-name'); 

    faulDisplays.name.home = document.querySelector('#home-faul-name');
    faulDisplays.name.guest = document.querySelector('#guest-faul-name'); 
    console.log(faulDisplays.name);
...
function changeFaul(score, home) {
    let display = home ? faulDisplays.home : faulDisplays.guest;

    changedigit(display.digits[0], Math.floor(score / 10));
    changedigit(display.digits[1], Math.floor(score % 10));
}

function changePor(score, home) {
    let display = home ? porDisplays.home : porDisplays.guest;
...
ipc.on('set-faul', (e, msg) => {
    changeFaul(msg.score, msg.home);
});
ipc.on('set-por', (e, msg) => {
    changePor(msg.score, msg.home);
});
...
ipc.on('set-por-name', (e, msg) => {
    porDisplays.name[msg.home ? 'home' : 'guest'].innerText = msg.changeTo;
});
ipc.on('set-faul-name', (e, msg) => {
    console.log("SET FAUL NAME:", msg);

    const target = msg.home
        ? faulDisplays.name.home
        : faulDisplays.name.guest;

    if (target) {
        target.innerText = msg.changeTo;
    } else {
        console.error("guest-faul-name not found");
    }
});
ipc.on('scale', (e, msg) => {
...
ipc.on('toggle-faul', () => {
    const el = document.getElementById("guest-faul");
