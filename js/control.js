...
let data = [{
    ...
    home: {
        ...
        faulDisplay: HTMLDivElement,
        porDisplay: HTMLDivElement,
...
function newscoreboardtab(name) {
    ...
    // Set team name controls
    tr.controls.querySelector('#home-faul-name').addEventListener('input', (e) => {
        changeFaulName(name, true, e.currentTarget.value);
    });
    tr.controls.querySelector('#guest-faul-name').addEventListener('input', (e) => {
        changeFaulName(name, false, e.currentTarget.value);
    });
    // Set team name controls
    tr.controls.querySelector('#home-por-name').addEventListener('input', (e) => {
        changePeriodName(name, true, e.currentTarget.value);
    });
    tr.controls.querySelector('#guest-por-name').addEventListener('input', (e) => {
        changePeriodName(name, false, e.currentTarget.value);
    });
    ...
    teamfaulcontrols(data[name].home, tr.controls.querySelector('#home-faul-controls'), true);
    teamfaulcontrols(data[name].guest, tr.controls.querySelector('#guest-faul-controls'), false);

    teamporcontrols(data[name].home, tr.controls.querySelector('#home-por-controls'), true);
    teamporcontrols(data[name].guest, tr.controls.querySelector('#guest-por-controls'), false);
    /**
     * 
    ...
    function teamfaulcontrols(setOn, attachTo, home) {
        setOn.faulDisplay = attachTo.querySelector('.team-faul');
        attachTo.querySelector('.increase-faul').addEventListener('click', () => { changeFaul(name, home, 1); });
        attachTo.querySelector('.decrease-faul').addEventListener('click', () => { changeFaul(name, home, -1); });
    }
    function teamporcontrols(setOn, attachTo, home) {
        setOn.porDisplay = attachTo.querySelector('.team-por');
        attachTo.querySelector('.increase-por').addEventListener('click', () => { changePor(name, home, 1); });
        attachTo.querySelector('.decrease-por').addEventListener('click', () => { changePor(name, home, -1); });
    }
    return tr;
}
...
function changeFaul(name, home, changeBy) {
    let setOn = data[name][home ? 'home' : 'guest'];
    setOn.currentFaul = gir(setOn.currentFaul + changeBy, 0, 99);
    setOn.faulDisplay.innerText = setOn.currentFaul.toString().padStart(2, '0');
    ipcToScoreboard(name, 'set-faul', { score: setOn.currentFaul, home: home });
}

function changePor(name, home, changeBy) {
    let setOn = data[name][home ? 'home' : 'guest'];
    setOn.currentPor = gir(setOn.currentPor + changeBy, 0, 99);
    setOn.porDisplay.innerText = setOn.currentPor.toString().padStart(2, '0');
    ipcToScoreboard(name, 'set-por', { score: setOn.currentPor, home: home });
}
/**
...
function changePeriodName(sbid, home, changeTo) {
    ipcToScoreboard(sbid, 'set-por-name', { home: home, changeTo: changeTo });


}

function changeFaulName(sbid, home, changeTo) {
    ipcToScoreboard(sbid, 'set-faul-name', { home: home, changeTo: changeTo });
        ...
        case 'home':
            switch (msg.arg) {
                ...
                case 'decrease':
                    ...
                    break;
                case 'increase-faul':
                    changeFaul(current, true, 1);
                    break;
                case 'decrease-faul':
                    changeFaul(current, true, -1);
                    break;
                 case 'increase-por':
                    changePor(current, true, 1);
                    break;
                case 'decrease-por':
                    changePor(current, true, -1);
                    break;
...
function toggleFaul() {
    // كنرسل أمر للـ main process
    ipc.send('relay', current, 'toggle-faul');
  }
